...
### Vérification
	...
	if (abs(VT-VA-VE)>1e-9) and (NbrClasses!=1) :
		print('Attention pb de calcul pour les variances (VT<>VA+VE)!\n VT={0}, VA={1}, VE={2}, difference={3}'.format(VT,VA,VE, VT-VA-VE))
...
# Vérifications
	...
	if (abs(np.sum(np.sum(CA+CE-CT)))>1e-9) and (NbrClasses!=1) :
		...
		if abs(np.trace(CT)-t)>1e-9:
			print('Attention pb de calcul sur la matrice de covariance totale CT!\n')
			CT=np.zeros((NbrVariables, NbrVariables))
		if abs(np.trace(CA)-a)>1e-9:
			print('Attention pb de calcul sur la matrice de covariance intraclasses CA!\n')
			CA=np.zeros((NbrVariables, NbrVariables))
		if abs(np.trace(CE)-e)>1e-9:
			print('Attention pb de calcul sur la matrice de covariance interclasses CE!\n')
			CE=np.zeros((NbrVariables, NbrVariables))
